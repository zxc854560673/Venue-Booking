...
namespace Venue_booking
{
    ...
    public class test
    {
        public static void log(string a) { System.Diagnostics.Debug.WriteLine(a); WriteMessage(mi.local+@"log\test.txt", a); }
        ...
        public static void forPause() { return; }
    }
    public class mydata {
        //public SqlConnection sConn;
        public bool data_connect() {
            string connSting;
            connSting = "server=localhost;database=wechat;User ID = zzzz;Password = zzzz;MultipleActiveResultSets=True";//server=localhost;database=wechat;Integrated Security=True;MultipleActiveResultSets=True
        ...
        public int data_change_self(string sql_string,Dictionary<string,object> dic=null) {
            try
            {
                ...
                {
                    if (dic != null)foreach (var z in dic)
                    {
        ...
        public object[] get_user_data(string user, string target,int yeshu=0) {
            switch (target) {
                case "mybook":
                    ...
                    string search = "SELECT wc_recoder.id,building_name,room_id,room_name,apply_time,apply_for_time,start_time,end_time,phone_num,reason,organization,reply_time,status FROM (SELECT TOP (@yeshu + 10) row_number() OVER (ORDER BY id desc)n,* FROM wc_recoder where applyer_id like @user_id)wc_recoder,wc_building WHERE n > @yeshu and wc_recoder.room_id=wc_building.id ORDER BY n";
        ...
        public bool have_identify(string user_id, string room_id) {
            int cl;
            var io = new Dictionary<string, object>();
            if (!int.TryParse(room_id, out cl)) return false;
            io.Add("room_id", cl);
            if (!int.TryParse(user_id, out cl)) return false;
            io.Add("user_id", cl);
            var a = mi.wc.data_search_self("Select can_use from wc_building where id=@room_id",io);
            if (a == null) return false;
            var c = mi.wc.data_search_self("Select wc_identify from wc_user_all where id=@user_id",io);
            if (c == null) return false;
            var z = int.Parse(a[0]["can_use"]);
            var q = int.Parse(c[0]["wc_identify"]);
            if (z > -1 && q >= z) return true;
            else return false;
        }
        public int getIdentifyById(string user_id) {
            var io = new Dictionary<string, object>();
            int cl;
            if (!int.TryParse(user_id, out cl)) return -1;
            io.Add("id", cl);
            var a = mi.wc.data_search_self("Select wc_identify from wc_user_all where id=@id", io);
            return int.Parse(a[0]["wc_identify"]); ;
        }
